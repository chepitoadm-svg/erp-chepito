-- =============================================================================
-- Agrega el destino 'CAS' (Casa) a la planilla: colaboradores de la casa (ej.
-- empleada doméstica) van a su propio centro de costo, no a las panaderías.
-- El centro 'CAS' (Casa) ya existe. CAS es destino fijo (no entra al reparto DIV).
-- =============================================================================

alter table public.colaborador_destino drop constraint if exists colaborador_destino_destino_check;
alter table public.colaborador_destino add constraint colaborador_destino_destino_check
  check (destino in ('TAL','CH1','CH2','DIV','CAS'));

alter table public.planilla_lineas drop constraint if exists planilla_lineas_destino_check;
alter table public.planilla_lineas add constraint planilla_lineas_destino_check
  check (destino in ('TAL','CH1','CH2','DIV','CAS'));
...
do $$ begin raise notice 'destino CAS (Casa) habilitado en planilla.'; end $$;
