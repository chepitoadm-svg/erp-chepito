...
import { anularGasto, type FormState } from "@/app/(app)/gastos/actions";

const inicial: FormState = {};

export default function AnularGasto({ id }: { id: string }) {
  const [abierto, setAbierto] = useState(false);
  const [state, formAction, pending] = useActionState(anularGasto, inicial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-xs text-red-700">Revierte el asiento del gasto.</p>
      <input
        type="text"
        name="motivo"
        required
        minLength={3}
        placeholder="Ej: cargado a la panadería equivocada"
