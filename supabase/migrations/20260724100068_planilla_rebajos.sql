-- =============================================================================
-- Rebajos de planilla (columna "Total Rebajos" del CSV): incapacidades y
-- ausencias que NO se pagan → reducen el salario devengado. El salario efectivo
-- del colaborador = salario_base − rebajos. La CCSS del CSV ya viene calculada
-- sobre el salario ajustado. Neto = base − rebajos + adicional − CCSS − adelanto.
-- =============================================================================

alter table public.planilla_lineas add column if not exists rebajos numeric(18,2) not null default 0;

-- guardar: acepta y persiste rebajos
...
begin
  ...
  insert into public.planilla_lineas
    (planilla_id, clave, cedula, nombre, puesto, tiene_ccss, destino,
     salario_base, ccss_obrero, cargas_patronal, pago_adicional, adelanto, rebajos)
  select v_id, x.clave, x.cedula, x.nombre, x.puesto, coalesce(x.tiene_ccss,false),
         ...
         coalesce(x.cargas_patronal,0), coalesce(x.pago_adicional,0), coalesce(x.adelanto,0), coalesce(x.rebajos,0)
    from jsonb_to_recordset(coalesce(p_lineas,'[]'::jsonb)) as x(
      ...
      salario_base numeric, ccss_obrero numeric, cargas_patronal numeric, pago_adicional numeric, adelanto numeric, rebajos numeric);
...
-- neto efectivo (saldo a pagar) = base − rebajos + adicional − CCSS − adelanto
...
-- postear: salario efectivo = base − rebajos
...
begin
  ...
  select coalesce(round(sum(ccss_obrero),2),0), coalesce(round(sum(cargas_patronal),2),0), coalesce(round(sum(adelanto),2),0),
         coalesce(round(sum(salario_base - rebajos + pago_adicional - ccss_obrero),2),0)
...
do $$ begin raise notice 'planilla: rebajos (incapacidad/ausencias) restados del salario.'; end $$;
