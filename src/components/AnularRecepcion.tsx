...
import { anularRecepcion, type FormState } from "@/app/(app)/compras/actions";

const inicial: FormState = {};

export default function AnularRecepcion({ id }: { id: string }) {
  const [abierto, setAbierto] = useState(false);
  const [state, formAction, pending] = useActionState(anularRecepcion, inicial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-xs text-red-700">
        Solo se anula si no fue facturada: revierte el asiento y saca la mercadería.
      </p>
      <input
        type="text"
        name="motivo"
        required
        minLength={3}
        placeholder="Ej: entrega equivocada"
