...
import { anularFactura, type FormState } from "@/app/(app)/compras/actions";

const inicial: FormState = {};

export default function AnularFactura({ id }: { id: string }) {
  const [abierto, setAbierto] = useState(false);
  const [state, formAction, pending] = useActionState(anularFactura, inicial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-xs text-red-700">
        Revierte el asiento, saca la mercadería del inventario y anula la CxP.
      </p>
      <input
        type="text"
        name="motivo"
        required
        minLength={3}
        placeholder="Ej: factura equivocada"
