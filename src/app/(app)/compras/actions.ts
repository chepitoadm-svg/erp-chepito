...
export async function subirComprobante(
  ...
  const supabase = await createClient();
  const res = await ingestarComprobante(supabase, xmlComp, xmlResp);
  if (!res.ok) {
    return {
      error:
        res.code === "procesado"
          ? "Ese comprobante ya generó una factura. Anulá esa factura si querés rehacerlo."
          : limpiar(res.error),
    };
  }

  revalidatePath("/compras/ingestor");
  redirect(`/compras/ingestor/${res.id}`);
}
