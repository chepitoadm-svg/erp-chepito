...
import { anularAjuste, type FormState } from "@/app/(app)/inventario/actions";

const inicial: FormState = {};

export default function AnularAjuste({ id }: { id: string }) {
  const [abierto, setAbierto] = useState(false);
  const [state, formAction, pending] = useActionState(anularAjuste, inicial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-xs text-red-700">
        No borra nada: revierte el asiento y reingresa/saca las existencias.
      </p>
      <input
        type="text"
        name="motivo"
