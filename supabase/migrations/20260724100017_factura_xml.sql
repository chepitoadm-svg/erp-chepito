-- =============================================================================
-- Fase 3 (fix): alta de factura de compra desde XML con montos EXACTOS.
--
-- fn_crear_factura (manual) recalcula el IVA como base × tarifa del artículo.
-- Eso no sirve para el XML real, que trae tasas mezcladas (1% canasta básica,
-- 13%), impuestos específicos que van al costo, descuentos y otros cargos.
-- fn_crear_factura_xml recibe la base (costo, ya con el específico incluido) y
-- el IVA acreditable EXACTOS por línea, y NO recalcula: así el total calza
-- exacto con el TotalComprobante del comprobante.
--   Es caso A (la factura trae la mercadería): exige bodega de ingreso.
-- =============================================================================
...
begin
  ...
  if auth.uid() is not null and not public.soy_administrador()
    ...
    raise exception 'No podés ingresar mercadería en esa bodega.'; end if;
  ...
  for r in select * from jsonb_array_elements(p_lineas) loop
    i := i + 1;
    v_base := round((r->>'base_imponible')::numeric, 2);   -- costo (ya con específico)
    v_ivam := round((r->>'iva_monto')::numeric, 2);        -- IVA acreditable exacto
    ...
    values (v_id, i, nullif(btrim(coalesce(r->>'codigo_comercial','')),''),
            ...
            v_base,
            nullif(btrim(coalesce(r->>'iva_tarifa_id','')),'')::uuid,
            v_ivam,
...
grant execute on function public.fn_crear_factura_xml(uuid, uuid, text, date, text, int, jsonb) to authenticated;

do $$
begin
  raise notice 'fn_crear_factura_xml lista (montos exactos del comprobante).';
end $$;
