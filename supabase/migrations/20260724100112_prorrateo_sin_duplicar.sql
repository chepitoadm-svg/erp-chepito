-- =============================================================================
-- "Rehacer prorrateo" no debe duplicar. Antes fn_generar_prorrateo creaba un
-- borrador nuevo cada vez; si el usuario confirmaba varios, el centro quedaba
-- prorrateado 2+ veces (Taller salía negativo). Ahora, antes de generar:
--   - Descarta cualquier BORRADOR previo de ese centro+periodo (rehacer real).
--   - Si ya hay uno CONFIRMADO, AVISA (hay que anularlo antes de rehacer).
-- Un prorrateo "es de" un centro si tiene una línea de vaciado con ese centro.
-- (Se recrea la función completa —igual que la migr. 100110— con esa guarda.)
-- =============================================================================
...
begin
  ...
  -- No duplicar: si ya hay un prorrateo CONFIRMADO de este centro ese mes, avisar.
  if exists (
  ...
  -- Descartar borradores previos de este centro+periodo (así "rehacer" reemplaza).
...
do $$ begin raise notice 'fn_generar_prorrateo: rehacer descarta borrador previo y avisa si hay confirmado.'; end $$;
