-- =============================================================================
-- Al postear la planilla se generan DOS asientos en un solo paso:
--  1) PROVISIÓN: salarios por pagar por el monto COMPLETO (bruto − CCSS obrero),
--     sin tocar la CxC. La CxC del adelanto sigue viva.
--  2) PAGO SIMULADO DEL ADELANTO: Debe Salarios por pagar / Haber CxC (11-30-11)
--     por el total de adelantos → simula que esa parte ya se pagó (con el adelanto
--     que el colaborador recibió antes) y deja la CxC en 0.
-- El saldo en efectivo que queda por pagar = bruto − CCSS − adelantos.
-- =============================================================================

alter table public.planilla add column if not exists adelanto_asiento_id uuid references public.asientos(id);
...
declare
  ...
  v_ret numeric; v_apo numeric; v_adel numeric; v_spp numeric;
...
begin
  ...
  select coalesce(round(sum(ccss_obrero),2),0), coalesce(round(sum(cargas_patronal),2),0), coalesce(round(sum(adelanto),2),0),
         coalesce(round(sum(salario_base + pago_adicional - ccss_obrero),2),0)   -- salarios por pagar COMPLETO
    into v_ret, v_apo, v_adel, v_spp
    from public.planilla_lineas where planilla_id = p_planilla;
  ...
  -- Asiento 2: pago simulado del adelanto (Debe Salarios por pagar / Haber CxC).
  if v_adel > 0 then
...
begin
  ...
  if v_adel is not null then
    perform public.fn_anular_asiento_auto('planilla_adelanto', p_planilla, p_motivo);
    update public.planilla set adelanto_asiento_id = null where id = p_planilla;
  end if;
...
do $$ begin raise notice 'planilla: provisión + pago simulado del adelanto en un solo posteo.'; end $$;
