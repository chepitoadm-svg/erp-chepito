...
import { anularCierre, type FormState } from "@/app/(app)/inventario/actions";

const inicial: FormState = {};

export default function AnularCierre({ id }: { id: string }) {
  const [abierto, setAbierto] = useState(false);
  const [state, formAction, pending] = useActionState(anularCierre, inicial);
  ...
  return (
    <form action={formAction} className="flex flex-col gap-2 rounded-md border border-red-200 bg-red-50 p-3">
      ...
      <p className="text-xs text-red-700">Revierte el asiento y el ajuste de kardex del cierre.</p>
      <input
        type="text"
        name="motivo"
        required
        minLength={3}
        placeholder="Ej: conteo mal digitado"
